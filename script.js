  ...
  try {
    ...
    for (let i = 0; i < 5 && tries < 100; i++) {
      ...
      if (
        !random.image_id ||
        !random.artist_title ||
        usedArtists.has(random.artist_title)
      ) {
        console.log(
          "Skipping artwork - missing image_id, artist_title, or duplicate artist"
        );
        i--;
        tries++;
        continue;
...
function checkAnswers() {
  ...
  if (allCorrect) {
    ...
    showPopup(
      `${correctCount} out of ${currentArtworks.length} correct. Try again!`
    );
  }
}
